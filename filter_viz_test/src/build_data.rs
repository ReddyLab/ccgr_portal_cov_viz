...
fn select_assembly<'a>(
    ...
    let mut assembly_info = match assembly_name {
        Some(x) => match x.as_str() {
            ...
            _ => {
                eprintln!("Invalid genome {}. Must be \"GRCH37\" or \"GRCH38\"", x);
                panic!();
            }
        },
        None => {
            eprintln!("No genome set. Genome must be \"GRCH37\" or \"GRCH38\"");
            panic!();
        }
    };
...
pub fn build_data(options: &Options, client: &mut Client) -> Result<CoverageData, Error> {
    let bucket = |size: u32| size / options.bucket_size.unwrap();
    ...
    Ok(CoverageData {
        ...
        bucket_size: options.bucket_size.unwrap(),
